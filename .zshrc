...
PS1='%B%K{166}%F{235}%n@%m%k%f:%K{166}%F{235}%~%k%f λ '
...
source $HOME/Extras/zsh-autosuggestions/zsh-autosuggestions.zsh
...
function gradleDeploy {
  if ./gradlew clean build -Dprops=/home/$USER/uportal/uportal/filters/local.properties; then
    sleep 1
    WARPATH=`readlink -f $(find . -name '*.war' -type f)`
    cd ~/uportal/uportal
    sleep 1
